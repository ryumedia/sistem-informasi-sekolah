...
import { ArrowLeft, Loader2, BarChart } from "lucide-react";

export default function SiswaPerkembanganPage() {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        ...
        if (!snapshotSiswa.empty) {
          ...
          // Jika user login tapi bukan siswa (misal guru iseng buka link ini), kembalikan ke home
          router.push("/"); 
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchMaster = async () => {
      try {
        ...
        // Fetch Kriteria Nilai (Khusus kategori "Nilai Perkembangan")
  ...
  // 3. Fetch Nilai Perkembangan Siswa
  useEffect(() => {
    ...
    const fetchNilai = async () => {
      setLoadingData(true);
      try {
        // Ambil nilai_perkembangan berdasarkan siswa dan semester
        const qNilai = query(
        ...
        // Kumpulkan tahapId untuk mengambil data Lingkup
        ...
        snapNilai.forEach(doc => {
          ...
          rawNilai.push({ id: doc.id, ...d });
        });

        // Fetch Tahap Perkembangan (untuk mendapatkan nama Lingkup)
        const tahapMap = new Map<string, any>();
        if (tahapIds.size > 0) {
            const ids = Array.from(tahapIds);
            // Chunking jika ID lebih dari 10 (batasan query 'in' Firestore)
            for (let i = 0; i < ids.length; i += 10) {
                const chunk = ids.slice(i, i + 10);
                const qTahap = query(collection(db, "tahap_perkembangan"), where(documentId(), "in", chunk));
                const snapTahap = await getDocs(qTahap);
                snapTahap.forEach(doc => {
                    tahapMap.set(doc.id, doc.data());
                });
            }
        }

        // Gabungkan Data Nilai dengan Lingkup
        const merged = rawNilai.map(item => {
            const tahap = tahapMap.get(item.tahapId);
            return {
                ...item,
                lingkup: tahap ? tahap.lingkup : "Lainnya",
                deskripsi: item.namaTahap || tahap?.nama || "-"
            };
        });

        // Urutkan berdasarkan Lingkup, lalu Deskripsi
        merged.sort((a, b) => {
    ...
    <div className="min-h-screen bg-gray-50 flex justify-center">
      <div className="w-full max-w-lg bg-white min-h-screen shadow-xl flex flex-col">
        ...
        <header className="bg-white p-4 shadow-sm sticky top-0 z-10 flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-800">Nilai Perkembangan</h1>
        </header>

        <div className="p-4 space-y-4">
            {/* Semester Selector */}
            <div className="bg-purple-50 p-3 rounded-xl border border-purple-100">
                <label className="text-xs font-medium text-purple-800 mb-1 block">Pilih Semester</label>
                <select 
                    value={selectedSemester} 
                    onChange={(e) => setSelectedSemester(e.target.value)}
                    className="w-full bg-white border border-purple-200 text-gray-700 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block p-2.5 outline-none"
                >
                    {semesterList.map(s => (
            ...
            {loadingData ? (
                ...
                <div className="text-center py-10 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                    <BarChart className="w-10 h-10 text-gray-300 mx-auto mb-2" />
                    <p className="text-gray-500 text-sm">Belum ada data nilai untuk semester ini.</p>
                </div>
            ) : (
                <div className="space-y-4 pb-8">
                    {nilaiList.map((item, index) => {
                        // Tampilkan Header Lingkup jika berbeda dengan item sebelumnya
                        const showHeader = index === 0 || nilaiList[index - 1].lingkup !== item.lingkup;
                        return (
                            <div key={item.id}>
                                {showHeader && (
                                ...
                                <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm flex justify-between items-center gap-4 hover:shadow-md transition">
                                    <div className="flex-1">
                                        <p className="text-sm text-gray-700 font-medium leading-snug">{item.deskripsi}</p>
                                    </div>
                                    <div className={`
