...
export default function SiswaTrilogiPage() {
  ...
  useEffect(() => {
    const fetchMaster = async () => {
      try {
        ...
        // Fetch Kriteria Nilai (Khusus kategori "Nilai Trilogi")
  ...
  useEffect(() => {
    ...
    const fetchNilai = async () => {
      ...
      try {
        ...
        // Urutkan berdasarkan Nama Trilogi, lalu Sub Trilogi
        rawNilai.sort((a: any, b: any) => {
            if (a.namaTrilogi < b.namaTrilogi) return -1;
            if (a.namaTrilogi > b.namaTrilogi) return 1;
            return (a.namaSubTrilogi || "").localeCompare(b.namaSubTrilogi || "");
        });
        ...
        <div className="p-4 space-y-4">
            ...
            {loadingData ? (
                ...
                <div className="text-center py-10 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                    <Triangle className="w-10 h-10 text-gray-300 mx-auto mb-2" />
                    <p className="text-gray-500 text-sm">Belum ada data nilai untuk semester ini.</p>
                </div>
            ) : (
                <div className="space-y-4 pb-8">
                    {nilaiList.map((item, index) => {
                        // Tampilkan Header Trilogi jika berbeda dengan item sebelumnya
                        const showHeader = index === 0 || nilaiList[index - 1].namaTrilogi !== item.namaTrilogi;
                        return (
                            <div key={item.id}>
                                {showHeader && (
